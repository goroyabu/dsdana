...
int ReadEventtree::set_read_branch()
{
    ...
    int asic = 0;
    while ( true ) {
	if ( read_branch<unsigned short int>( "hitnum"+std::to_string(asic) ) == anlcross::ANL_NG ) break; 
	read_branch<unsigned short int>( "cmn"+std::to_string(asic) );
	read_branch_array<unsigned short int>( "index"+std::to_string(asic),   64);
	read_branch_array<unsigned short int>( "adc"+std::to_string(asic),   64);
	++asic;
    }
