...
MergeAdjacentText::MergeAdjacentText() : ANLModuleBase("MergeAdjacentText", "1.0"), mDatabase(nullptr), m_histogram(nullptr)
{
    m_ndetector = 1;
    m_save_his = true;
}
void MergeAdjacentText::mod_init(int &status)
{
    using namespace evs;
    status = ANL_OK;
    //std::cout << "MergeAdjacentText::mod_init" << std::endl;
...
void MergeAdjacentText::mod_his(int &status)
{
    m_multipli_his = new TH2D("multipli_lv2", "multiplicity lv2;nsignal_x_lv2;nsignal_y_lv2", 50, -0.5, 49.5, 50, -0.5, 49.5);
    status = ANL_OK;
}
void MergeAdjacentText::mod_com(int &status)
{
    status = ANL_OK;
}
void MergeAdjacentText::mod_ana(int &status)
{
    using namespace std;
    using namespace bnk;
    status = ANL_OK;
...
void MergeAdjacentText::mod_endrun(int &status)
{
    m_multipli_his->Write();
    status = ANL_OK;
}
void MergeAdjacentText::mod_exit(int &status)
{
    //m_histogram->Delete();
    status = ANL_OK;
    //std::cout << std::endl;
}
int MergeAdjacentText::bnkDefAll()
{
    using namespace bnk;
...
int MergeAdjacentText::bnkGetAll()
{
    using namespace bnk;
...
int MergeAdjacentText::bnkPutAll()
{
    using namespace bnk;
    using namespace evs;
...
int MergeAdjacentText::clearVectorAll()
{
    m_nsignal_x_lv2 = 0;
    m_nsignal_y_lv2 = 0;
    m_detid_x_lv2.clear();
...
void MergeAdjacentText::extractOneDetector(int detid, std::vector<lv1data>& xdata, std::vector<lv1data>& ydata)
{
    using namespace std;
    xdata.clear();
    for(int i=0; i<m_nsignal_x_lv1; ++i){
...
int MergeAdjacentText::extractSignalsOverThreshold()
{
...
int MergeAdjacentText::convertLv1toLv2(int detid)
{
    using namespace std;
...
int MergeAdjacentText::fillHistogram(std::vector<lv1data> &data_list)
{
    using namespace std;
...
bool MergeAdjacentText::isAdjacent(int stripid, const std::vector<int>& stripid_list)
{
...
bool MergeAdjacentText::isAdjacent(int stripid, const std::vector<lv1data>& data_list)
{
